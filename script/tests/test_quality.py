...
class TestAssessContent:
    ...
    def test_long_content_without_hits_is_ok(self):
        # 长文标题含 captcha（技术教程）→ 内容充足，不误伤；短快讯无拦截词同样放行
        ...
        short = "快讯：某公司发布新品。\n\n价格待公布。"  # 短但无拦截词
        q2 = assess_content(short)
        assert q2.ok and not q2.low_quality


class TestCheckedText:
    def test_checked_text_mapping(self):
        # 拦截页 → BLOCKED 抛错；正常 → 无标记；可疑(长度在警告区间且有命中) → low_quality
        ...
        assert low is True and "安全验证" in reason
