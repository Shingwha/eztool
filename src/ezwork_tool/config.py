...
def legacy_paths() -> dict[str, str]:
    ...
    cands = {
        "doubao": [os.path.join(home, ".config", "doubao-websearch", "config.json")],
        "anysearch": [os.path.join(appdata, "anysearch-cli", "config.json")],
        "deepseek": [os.path.join(appdata, "deepseek-websearch", "config.json")],
        "fetch": [os.path.join(home, ".config", "ezwork-fetch", "config")],
    }
    out = {}
    for sec, paths in cands.items():
        for p in paths:
            if os.path.isfile(p):
                out[sec] = p
                break
...
def import_legacy(cfg: dict) -> list[str]:
    """导入旧工具配置到对应段，返回导入说明列表。"""
    imported = []
    for sec, path in legacy_paths().items():
        try:
            if sec == "fetch":  # 旧 fetch 是 INI 格式
                import configparser
                ...
                with open(path, encoding="utf-8") as f:
                    ini.read_file(f)
                data = {}
                if ini.has_option("ezwork-fetch", "providers"):
                    data["providers"] = [p.strip() for p in ini.get("ezwork-fetch", "providers").split(",") if p.strip()]
                if ini.has_option("ezwork-fetch", "timeout"):
                    data["timeout"] = ini.getint("ezwork-fetch", "timeout")
                if ini.has_section("firecrawl"):
                    for k in ("api_key", "timeout"):
                        if ini.has_option("firecrawl", k):
                            data.setdefault("firecrawl", {})[k] = ini.get("firecrawl", k) if k == "api_key" else ini.getint("firecrawl", k)
                if ini.has_section("jina"):
                    for k in ("api_key", "timeout"):
                        if ini.has_option("jina", k):
                            data.setdefault("jina", {})[k] = ini.get("jina", k) if k == "api_key" else ini.getint("jina", k)
            else:
                with open(path, encoding="utf-8") as f:
                    data = json.load(f)
            if not isinstance(data, dict):
                continue
        except (OSError, ValueError, configparser.Error) as e:  # noqa: F821
            continue
        # 只搬已知键（与默认结构对齐）
        known = DEFAULTS.get(sec, {})
        moved = {}
        for k, v in data.items():
            if k in known and v is not None:
                moved[k] = v
        if moved:
            set_key(cfg, sec, deep_merge(get_key(cfg, sec, {}), moved))
            imported.append(f"{sec} ← {path}（{len(moved)} 项）")
    return imported
